...
class Anchor:
    def __init__(self, message):
        ...
        self.master_number = message["MASTER_NUMBER"]
        ...
        self.Connect() # открытие socket-соединения с маяками
    ...
    def Connect(self):
        try:
            ...
            if not self.disconnect_flag: print(f"ANCHOR NUMBER {self.number} WITH NAME {self.name} [IP: {self.IP} ] CONNECTED")
            if self.disconnect_flag: print(f"ANCHOR NUMBER {self.number} WITH NAME {self.name} [IP: {self.IP} ] RECONNECTED")
        except:
    ...
    async def SetRfConfig(self, rf_config):
        ...
        except: print(f"ERROR SET RF_CONFIG ON ANCHOR NUMBER {self.number} WITH NAME {self.name} [IP {self.IP} ]")
    ...
    async def StartSpam(self):
        ...
        except: print(f"ERROR START SPAM ON ANCOR NUMBER {self.number} WITH NAME {self.name} [IP {self.IP} ]")

    async def Stop(self):
        try:
            self.socket.sendall(rm.build_RTLS_START_REQ(0))
            print(f"ANCHOR NUMBER {self.number} WITH NAME {self.name} [IP: {self.IP} ] STOP")
        except: print(f"ERROR STOP ANCHOR NUMBER {self.number} WITH NAME {self.name} [IP {self.IP} ]")
    ...
    async def AnchorHandler(self, log_buffer):
        while True:
            ...
            try:
                ...
                if message["type"] == "CS_TX": message["sender"] = message["receiver"]
